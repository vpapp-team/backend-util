...
exports.validateTimetablesLessons = (dataContent, dataLessons, dataUUID) => {
  ...
  // TODO: note that dataLessons are from the web parser
  if (dataLessons) throw new Error('!!Timetable with lessons');

  // TODO: note that dataLessons are from the database
  // TODO: can we just rename those to dataLessons since they get auto casted?
  if (dataContent) throw new Error('!!Timetable with content');

  // TODO note that dataUUID is identical with masterUUID for those from web parser - as long as its not a teacher or room plan
  // TODO: note that we prefer masterUUID over dataUUID since that means its not originally from the dataUUID table but from the masterUUID table
  ...
  return dataLessons.map(a => a instanceof LESSON ? a : new LESSON(a, a.masterUUID || dataUUID));
};
