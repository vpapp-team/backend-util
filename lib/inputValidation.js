...
// Validate a Lesson
exports.validateLesson = (dataLesson, varname, canBeNull = false) => {
  if (!TYPES) TYPES = require('backend-types');
  const LESSON = TYPES.get('Data').get('Lesson');
  if (dataLesson instanceof LESSON) return dataLesson;
  try {
    return new LESSON(dataLesson);
  } catch (e) {
    throw new TypeError(`${varname}${canBeNull ? ' not null nor a valid Lesson: ' : ' invalid: '}${e.message}`);
  }
};
...
exports.validateLessonDiscrim = (dataLesson, varname = 'lesson', canBeNull = false) => {
  if (!TYPES) TYPES = require('backend-types');
...
exports.validateClassDiscrim = (dataClass, varname = 'class', canBeNull = false) => {
  if (!TYPES) TYPES = require('backend-types');
...
// Validate a JSON
exports.validateJSON = (data, varname) => {
  try {
    return JSON.parse(data);
  } catch (e) {
    throw new TypeError(`JSON ${varname} invalid: ${e.message}`);
  }
};
...
exports.validateLessonRangesTime = dataRange => {
  if (typeof dataRange === 'string') {
    dataRange = dataRange.trim();
    if (!dataRange.match(RANGE_REGEXP)) throw new TypeError('time string has wrong format');
    const parts = dataRange.split('-');
    return {
      start: parts[0] || null,
      end: parts[1] || null,
    };
  } else if (typeof dataRange === 'object') {
    if (typeof dataRange.start !== 'string') throw new TypeError('time.start not a string');
    if (dataRange.start && !dataRange.start.match(RANGE_PART)) throw new TypeError('time has invalid start');
    if (typeof dataRange.end !== 'string') throw new TypeError('time.end not a string');
    if (dataRange.end && !dataRange.end.match(RANGE_PART)) throw new TypeError('time has invalid end');
    return {
